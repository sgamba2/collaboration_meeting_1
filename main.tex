...
scrivere chi ha partecipato
what i am reporting is based on work done by...
contesto:we are going to the first station being read, we should know how to read detector
integratione e commissioning
testing the daq works
learning about errors and data transmission
...
to be done: implementare lo slow control dei lowhigh voltage
    \begin{columns}
        ...
        \begin{column}{0.5\framewidth}
            ...
            \item A pulser implemented in the DRACs is sending pulses to the preamps (CAL side) at 50 kHz (variabile ma si e testato cosi);
            \item Pulses are digitized at 40 MHz (we were not able to buy adc at 50 Mhz);
...
%\begin{column}{0.5\framewidth}
 \begin{figure}[H]
   \centering
   \includegraphics[width= 1.0\columnwidth]{figures/png/Screenshot_20240308_165327.png}
   \label{fig:djn}
 \end{figure}
\begin{itemize}
\item Pulses are separated by 20 $\mu$s (1/$f_{gen}$) - trying to learn how to use it;
...
\item The timing of the
readout is uncorrelated with the generator timing sequences$\rightarrow$number of readout pulses is variable;
...
\item waveform shape;
\item first sample ($Wf_{thr}$ > 5) distribution;
\item charge distribution;
\item pulse height distribution;
\item tail charge distribution;
\item $\Delta t$ distribution between hits.
\item Error found : XXX
\end{itemize}
\end{column}
\begin{column}{0.5\framewidth}
 \begin{figure}[H]
...
\end{column}
\begin{column}{0.5\framewidth}
cross talks, in odd channels and asymmetric
\end{column}
\end{columns}
\end{frame}

\begin{frame}{features vs channel}
\begin{columns}
\begin{column}{0.5\framewidth}
\end{column}
\begin{column}{0.5\framewidth}
we look at the mean value and from there we establish the critical channels
\end{column}
\end{columns}
\end{frame}
\begin{frame}{analysis of the charge and pulse height}
\end{frame}
\begin{frame}{tmean}
\end{frame}
